...
// ---------------- SCREEN 2 PINS ----------------
#define SCREEN1_CS   33
#define SCREEN2_CS   25
#define SCREEN2_DC   26
#define SCREEN2_RST  32

TFT_eSPI tft = TFT_eSPI();
RTC_DS3231 rtc;
Adafruit_ILI9341 tft2 = Adafruit_ILI9341(SCREEN2_CS, SCREEN2_DC, SCREEN2_RST);
...
// ---------------- SETTINGS UI STATE ----------------
int settingsMainIndex = 0;      // 0 = Set Date, 1 = Set Time
...
// ---------------- COLORS ----------------
#define UI_WHITE        0xFF7A
#define UI_BG           0xFF7A
#define UI_CARD         0xFF9C
#define UI_BLACK        0x2124
#define UI_BLUE         0x19AA
#define UI_DARKBLUE     0x10A6
#define UI_SOFTBLUE     0xDEFB
#define UI_BORDER       0xC618
#define UI_BROWN        0x6A00
#define UI_TAPE_FILL    0xFF38
#define UI_TODAY_COL    0xFF59
#define UI_LIGHTGRAY    0xDEDB
#define UI_MIDGRAY      0x94B2
#define UI_DARKGRAY     0x632C
#define UI_SIDEBAR      0xF6F7
...
// ---------------- HELPERS ----------------
void prepareScreen1Bus() {
  pinMode(SCREEN1_CS, OUTPUT);
  pinMode(SCREEN2_CS, OUTPUT);
  digitalWrite(SCREEN2_CS, HIGH);   // keep screen 2 off the SPI bus
}
...
void openTimeDropdownForCurrentField() {
  ...
  else if (timeFieldIndex == 1) timeDropdownIndex = editMinute;
...
void settingsHandleNext() {
  if (!dateEditorOpen && !timeEditorOpen) {
    settingsMainIndex++;
    if (settingsMainIndex > 1) settingsMainIndex = 0;
    return;
  }
...
void drawStatsHeader() {
  ...
  for (int i = 0; i < 7; i++) {
    if (i == todayCol) tft.setTextColor(UI_DARKBLUE, UI_TODAY_COL);
    else tft.setTextColor(UI_DARKGRAY, UI_BG);
...
void drawSettingsButtonCard(int x, int y, int w, int h, const char* label, bool selected) {
  ...
  if (selected) tft.drawRoundRect(x + 1, y + 1, w - 2, h - 2, 8, border);
...
void drawSettingsPage() {
  ...
  drawSettingsButtonCard(18, 56, 126, 34, "Set Date", settingsMainIndex == 0 && !dateEditorOpen && !timeEditorOpen);
  drawSettingsButtonCard(176, 56, 126, 34, "Set Time", settingsMainIndex == 1 && !dateEditorOpen && !timeEditorOpen);

  if (dateEditorOpen) {
    drawDateEditor();
  } else if (timeEditorOpen) {
    drawTimeEditor();
  } else {
    tft.setTextFont(2);
    tft.setTextColor(UI_DARKGRAY, UI_BG);
    tft.drawCentreString("SELECT opens highlighted option", 160, 114, 2);
    tft.drawCentreString("NEXT moves between Set Date / Set Time", 160, 132, 2);
    ...
    tft.fillRoundRect(18, 154, 284, 28, 8, UI_CARD);
    tft.drawRoundRect(18, 154, 284, 28, 8, UI_BORDER);
    tft.setTextFont(2);
    tft.setTextColor(UI_BLACK, UI_CARD);
    tft.drawString("Date: " + currentDate, 28, 162);

    tft.fillRoundRect(18, 190, 284, 28, 8, UI_CARD);
    tft.drawRoundRect(18, 190, 284, 28, 8, UI_BORDER);
    tft.setTextFont(2);
    tft.setTextColor(UI_BLACK, UI_CARD);
    tft.drawString("Time: " + currentTime, 28, 198);
  }
}
...
void drawScreen() {
  ...
  if (menuOpen) drawSidebar();
}

// ---------------- SECOND SCREEN ----------------
void initStaticSecondScreen() {
  pinMode(SCREEN1_CS, OUTPUT);
  pinMode(SCREEN2_CS, OUTPUT);
  ...
  SPI.begin(18, 19, 23, SCREEN2_CS);

  digitalWrite(SCREEN1_CS, HIGH);
  digitalWrite(SCREEN2_CS, LOW);
...
void setup() {
  ...
  // Draw the static second screen once
  initStaticSecondScreen();

  // Then run screen 1 normally
  prepareScreen1Bus();
  tft.init();
  tft.setRotation(1);
  tft.fillScreen(UI_BG);
